...
const Decodable = (data, struct, enableConvert) => {
    if (!data &&
        typeof data !== 'object' &&
    ...
    return Object.keys(data).reduce((acc, el) => {
        if (data[el] && typeof data[el] === 'object') {
            if (!Array.isArray(data[el])) {
                const ob = exports.Decodable(data[el], struct[el], enableConvert = false);
                ...
                const arr = data[el].map((element) => {
                    if (element &&
                        ...
                        return exports.Decodable(element, struct[el][0], enableConvert);
                    }
